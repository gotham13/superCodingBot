...
NAME,JUDGE,HANDLE,SELECTION,HOLO,SOLO,POLO,XOLO,REMOVER,UPDA,QSELCC,LANG,CODE,DECODE,TESTCASES,RESULT,OTHER,FILE,FILETEST,GFG1,GFG2,GFG3,DB,CF,SCHED,REMNOTI,QSELCF,SUBSEL,SUB,UNSUB,MSG,BDC=range(32)
...
# SENDS UPCOMING CONTESTS DATA
def upcoming_sender(update, contest_list):
    i = 0
    s = ""
    keyboard = []
    keyboard1 = []
...
def upcoming(bot, update):
    ...
    try:
        jsonData = json.loads(rawData)
        searchResults = jsonData['objects']
        upcoming_sender(update, searchResults)
        upc = searchResults
    except:
        upcoming_sender(update, upc)
    return SCHED
